...
class sizeModule {
		...
		elements.forEach((item, index) => {
			...
			if (item.type == 'file') {
				...
				downloadFileNode.innerHTML = `
					<a href="https://raw.githubusercontent.com/${this.repositoryParameters.username}/${this.repositoryParameters.repository}/${this.repositoryParameters.branch}/${this.repositoryParameters.path}/${item.name}" class="tooltipped tooltipped-n" aria-label="Download file" download>
	...
	showRepositorySize(elements) {
		const repositoryTree = api.getRepositoryInformation(
			this.repositoryParameters.username,
			this.repositoryParameters.repository)

		repositoryTree.then((response) => {
			const sizeInformation = this.getReadableSizeUnit(response.size, true)
