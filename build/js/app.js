...
var parameters = _detection2.default.getRepositoryInformation();

if (parameters) {
	var response = api.getRepositoryContent(parameters.username, parameters.repository, parameters.branch, parameters.path);
	var elements = response.then(function (response) {
		var folders = [];
		var files = [];
		var others = [];
...
var _xregexpAll = __webpack_require__(6);
...
var detectionUtils = {
	...
	getRepositoryInformation: function getRepositoryInformation() {
		var url = window.location.pathname;
		var urlParts = url.split('/');
		urlParts.shift();
...
var getRepositoryContent = function getRepositoryContent(username, repository) {
	...
	return fetch(eval('`' + _constants2.default.GITHUB_API_CONTENT + '`')).then(function (response) {
		if (response.ok) {
...
var getRepositoryTree = function getRepositoryTree(username, repository) {
	...
	return fetch(eval('`' + _constants2.default.GITHUB_API_TREE + '`')).then(function (response) {
		if (response.ok) {
			return response.json();
		} else {
			console.error('WRONG RESPONSE');
